...
class _NearbyInstitutionsScreenState extends State<NearbyInstitutionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(settings.translate('nearby_institutions_title')),
        ...
        children: [
          ...
          if (_currentPosition != null)
            Positioned(
              ...
              child: Container(
                ...
                child: Text(
                  settings.translate('google_maps_key_warning'),
          ...
          Positioned(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _buildFilterChip(settings.translate('courthouses'), Icons.gavel_rounded),
                  _buildFilterChip(settings.translate('registry_offices'), Icons.badge_outlined),
                  _buildFilterChip(settings.translate('municipalities'), Icons.account_balance_outlined),
                ],
              ),
            ),
          ),
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              settings.translate('getting_location_loading'),
              style: const TextStyle(color: AppColors.text600, fontWeight: FontWeight.w500),
            ),
          ],
        ),
      ),
  ...
  Set<Marker> _buildMarkers() {
    ...
    return _mockInstitutions.map((inst) {
      return Marker(
        ...
        infoWindow: InfoWindow(title: settings.translate(inst.name)),
      );
    }).toSet();
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(settings.translate(inst.type).toUpperCase(), style: const TextStyle(color: AppColors.accent, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
              IconButton(onPressed: () => setState(() => _selectedInstitution = null), icon: const Icon(Icons.close, size: 18)),
            ],
          ),
          Text(settings.translate(inst.name), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.text900)),
          const SizedBox(height: 12),
          _buildDetailRow(Icons.schedule_outlined, "${settings.translate('working_hours_label')}: ${inst.workingHours}"),
          _buildDetailRow(Icons.phone_outlined, "${settings.translate('contact_label')}: ${inst.phone}"),
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: Text(settings.translate('get_directions'), style: const TextStyle(color: Colors.white)),
